...
def parse_stderr_contents(stderr_log_contents, step_contents, versions, size_list):
    ...
    try:
        for line in stderr_log_contents:
            line = line.strip('\n')
            # added. 27.Mar.2018 (for timestamp options)
...
def read_docker_info(docker_info_file):
    ...
    docker_info = ''
    try:
        with open(docker_info_file, mode='r') as f:
            docker_info = f.read()
    except Exception:
        print('docker info execution error')
    finally:
        return docker_info

def write_json(start_date, end_date, container_info_list, stderr_log_contents, step_contents, inputs_jobs, env_info, output_file, genome_version, parse_result, docker_info):

    template = OrderedDict()

    ### workflow section.
    platform = OrderedDict()
    platform['instance_type'] = env_info['instance_type']
    platform['region'] = env_info['region']
    platform['hostname'] = env_info['host_name']
    platform['docker_info'] = docker_info
...
def get_date(stderr_log_contents):
...
def main():
    ...
    parser.add_argument('--docker_ps', metavar='<docker_ps_file>', type=str, required=True)
    parser.add_argument('--docker_info', metavar='<docker_info_file>', type=str, required=True)
    parser.add_argument('--cwl_log', metavar='<cwl_log_file>', type=str, required=True)
    parser.add_argument('--cwl_input', metavar='<cwl_yaml_file>', type=str, required=True)
    # get arguments.
    args = parser.parse_args()
    docker_ps_log = args.docker_ps
    docker_info_file = args.docker_info
    stderr_log = args.cwl_log
    yaml_file = args.cwl_input

    ### read docker info contents.
    docker_info = ''
    docker_info = read_docker_info(docker_info_file)
    ...
    start_date = ''
    end_date = ''
    start_date, end_date = get_date(stderr_log_contents)
    ...
    ### write log
    write_json(start_date, end_date, container_info_list, stderr_log_contents, step_contents, inputs_jobs, env_info, output_file, genome_version, parse_result, docker_info)
